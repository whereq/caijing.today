...
- **Auth is optional.** News browsing is public (`OptionalUser`), unlike flowdesk. caijing.today has its **own Keycloak realm** (`caijing.today`, SPA client `caijing-spa`, admin role `cj-admin`) on the shared KeyToMarvel.com server — to be created there. Use `CurrentUser` only where a signed-in user is genuinely required.
